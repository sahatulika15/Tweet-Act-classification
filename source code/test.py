...
import batGenBert_test
from batGenBert_test import BatchGenerator
...
with tf.Session(config=tf.ConfigProto(gpu_options=gpu_options)) as sess:
    ...
    saver.restore(sess, './Models/task1/baselines/bestModel/best_epoch47')
    # ,feed_dict={lut_ph:emb_mat}
    ...
    for task_id in range(totTaskNum):
        ...
        for batches in range(vBgen.numBatches):
            x_batch, y_batch, sent_batch, dial_batch, taskdisc,exfeat, bat_inds = vBgen.nextBatch()
            ...
            VAcc+=a_bat
            all_pred.append(pre_bat)
            all_true.append(np.argmax(np.squeeze(y_batch, axis=0), axis=1))
            all_inds.append(bat_inds)
        VAcc/= vBgen.numBatches
        print('Task: {} | Accuracy {} \n'.format(task_id,VAcc))
        all_pred_c = np.concatenate(all_pred)
        all_true_c = np.concatenate(all_true)
        all_inds_c = np.concatenate(all_inds)
        target_names = ['class_' + str(i) for i in range(nClasses[task_id])]
        print(classification_report(all_true_c, all_pred_c, target_names=target_names, digits=4 ))
        print(confusion_matrix(all_true_c, all_pred_c))
        with open('./Infer/baselines/'+modelver+'.pkl', 'wb' ) as fd:
            pkl.dump([all_pred_c, all_true_c, all_inds_c],fd)
        sys.stdout.flush()
        del vBgen
